...
db <- odbcConnect("infobright-wybory2015prez1", uid="root", pwd="",DBMSencoding="UTF8",readOnlyOptimize=TRUE)
...
FROM
        (SELECT id,lp,kandydat,SUM(t.glosy) AS glosy FROM wyniki AS t GROUP BY id,lp,kandydat) AS w,
        obwody AS o,
        protokoly AS pu,
        protokoly AS pk,
        protokoly AS pn,
...
dfgminy$shortlista<-factor(dfgminy$kandydat,levels = c(
        "Braun Grzegorz Michał","Duda Andrzej Sebastian","Jarubas Adam Sebastian",
        "Komorowski Bronisław Maria", "Korwin-Mikke Janusz Ryszard", "Kowalski Marian Janusz",
        "Kukiz Paweł Piotr", "Ogórek Magdalena Agnieszka", "Palikot Janusz Marian", "Tanajno Paweł Jan",
        "Wilk Jacek "
        ),labels = c(
        "Braun",
        "Duda",
...
listy<-c("Komorowski","Duda","Kukiz","Korwin-Mikke","Ogórek")
kolory<-c("#ff7f0040","#377eb840","#4daf4a40","#984ea340","#e41a1c40")
koloryf<-c("#ff7f00","#377eb8","#4daf4a","#984ea3","#e41a1c")
...
plot(dfgminy$frekwencja[dfgminy$shortlista==listy[1]],dfgminy$rezultat.lista[dfgminy$shortlista==listy[1]],pch = ".",type="n",
     ...
     main="Wybory prezydenckie 2015 (I tura)",
...
for (i in 1:length(listy)) {
        ...
        plot(dfgminy$frekwencja[dfgminy$shortlista==listy[i]],dfgminy$rezultat.lista[dfgminy$shortlista==listy[i]],col=kolory[i],pch = ".",
                ...
                main=paste0("Wybory prezydenckie 2015 (I tura) - ",listy[i]),
                sub="(każdy punkt to jeden obwód wyborczy)")
        dev.off()
}
...
print(xyplot(rezultat.lista~frekwencja|woj,data=dfgminy[dfgminy$shortlista==listy[i],],pch=".",col=kolory[i],
       xlab="Frekwencja",ylab="Poparcie dla kandydata",xlim=c(0,1),ylim=c(0,1),
       main=paste0("Wynik kandydata ",listy[i]," w województwach"),
       sub="każdy punkt to jeden obwód wyborczy"
        ))
stopka()
dev.off()
...
hist(dfgminy$frekwencja[dfgminy$lp==1],breaks = 50,ylab="Liczba obwodów",xlab="Frekwencja",main="Frekwencja w obwodach",col="gold")
dev.off()

png(filename=paste0("frekwencja-woj.png"),width=809,height=655,type="windows",antialias="cleartype")        
histogram(~frekwencja|woj,data=dfgminy[dfgminy$lp==1,],type="count",breaks=25,xlab="Frekwencja",ylab="Liczba obwodów",main="Frekwencja w województwach",col="gold")
stopka()
dev.off()
...
f1<-hist(dfgminy$frekwencja[dfgminy$lp==4],breaks = 100,plot=FALSE)
f2<-hist(dfgminy$frek2[dfgminy$lp==4],breaks = 100,plot=FALSE)
